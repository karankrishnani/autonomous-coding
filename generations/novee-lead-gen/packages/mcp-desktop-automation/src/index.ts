#!/usr/bin/env node
/**
 * MCP Server for Novee Desktop App Automation
 *
 * This server exposes Playwright Electron automation tools that allow
 * the coding agent to launch, interact with, and test the desktop app.
 */
...
// Paths
const DESKTOP_APP_PATH = path.resolve(__dirname, '../../../apps/desktop');
...
const TOOLS: Tool[] = [
  {
    name: 'desktop_launch',
    description: 'Launch the Novee Electron desktop app and return a session ID',
    inputSchema: {
      type: 'object',
      properties: {
        env: {
...
async function handleDesktopLaunch(args: {
  ...
  if (!fs.existsSync(MAIN_JS_PATH)) {
    throw new Error(
      `Desktop app not built. Run 'pnpm --filter @novee/desktop build' first.\n` +
